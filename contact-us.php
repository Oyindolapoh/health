...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($_POST['full_name'])) {
        $error['full_name'] = "Enter Full Name";
    }
    ...
    if (empty($_POST['enquiry'])) {
        $error['enquiry'] = "Enter Information";
    }
    ...
    if (empty($error)) {
        $statement = $conn->prepare("SELECT * FROM enquiries WHERE email = :email");
    ...
    if (empty($error)) {
        $stmt = $conn->prepare("INSERT INTO enquiries (full_name, phone, email, enquiry, time_created, date_created) 
            VALUES (:full_name, :phone, :email, :enquiry, NOW(), NOW())");
        $stmt->execute([
...
<form method="POST" enctype="multipart/form-data">
    <div class="row clearfix">
        ...
        <div class="col-lg-12 col-md-12 col-sm-12 form-group">
            <textarea class="" id="enquiry" name="enquiry" placeholder="Enquiry"></textarea>
        </div>
